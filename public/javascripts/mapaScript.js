//https://www.mapbox.com/mapbox.js/example/v1.0.0/l-mapbox-marker/
	...
	function cargarPuntos(todos) {
		for(x in puntos){
			...
			if (puntos[x].salud==0 && todos==true){
			    ...
			    properties: {
			        title: 'No se detectó la enfermedad',
			        description: puntos[x].fechacaptura,
			        'marker-size': 'large',
			        "marker-color": '#90EE90'
			    ...
			    properties: {
			        title: 'Se detectó un mango enfermo',
			        description: puntos[x].fechacaptura,
			        'marker-size': 'large',
			        "marker-color": '#EAEA78'
			        //'marker-symbol': '' //https://www.mapbox.com/maki-icons/
			    ...
			    properties: {
			        title: 'Se detectaron 2 mangos enfermos',
			        description: puntos[x].fechacaptura,
			        'marker-size': 'large',
			        "marker-color": '#EAAE78'
			        //'marker-symbol': '' //https://www.mapbox.com/maki-icons/
			    ...
			    properties: {
			        title: 'Se detectaron mas de 2 mangos enfermos',
		...
		for(y in listaPoligonos){//Se agregan los poligonos al mapa
			if (listaPoligonos[y][1].salud==0 && todos==true){
			 	L.mapbox.featureLayer({
				    ...
				    properties: {
				        title: listaPoligonos[y][0].nombre,
				        description: listaPoligonos[y][0].comentario,
				        //"fill": '#EA7878'
				        "fill": '#90EE90'
				    }
				}).addTo(map);
	...
	$(document).ready(function(){
		...
		$( "#btnBuscar" ).click(function() {
			$( "#liUsuarios a" ).remove();
			$.post("http://localhost:3000/map/findUser",{nombreUsuario:$("#txtUsuario").val()})
				.done(function(data){
		...
		$("#liUsuarios").click(function(event){
			...
			window.location.href = "http://localhost:3000/map/user/"+event.target.text;
